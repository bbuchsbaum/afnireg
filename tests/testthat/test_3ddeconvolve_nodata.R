...
test_that("simple one-regressor model with -nodata works", {
  ...
  # Check that no input files are specified (but mask can still be present)
  expect_null(alm$cmd$cmdlines$input)
  # Mask can be specified even with nodata
  expect_equal(alm$cmd$cmdlines$mask, "./dummy_mask.nii")
