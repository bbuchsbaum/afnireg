...
test_that("IRESP output and TENT basis function work together", {
  ...
  model <- build_test_afni_model(
    design,
    onset ~ afni_hrf(condition, basis = "TENT", start = 0, stop = 18, nbasis = 10),
    sframe
  )
  ...
  # TENT basis should have nbasis functions
  expect_equal(attr(term$hrfspec$hrf, "nbasis"), 10)
})
...
test_that("amplitude modulated design with stim_times_IM works", {
  ...
  design <- data.frame(
    ...
    amplitude = rnorm(21, mean = 1, sd = 0.3),  # Trial-wise modulation
    run = 1
  )
