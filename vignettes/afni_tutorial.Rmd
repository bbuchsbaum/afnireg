...
afni_available <- nzchar(Sys.which("3dDeconvolve"))
```
...
- AFNI installed and `3dDeconvolve` available on your `PATH`.
- Model specification via `fmridesign` (event + baseline models).
- Optional: An `fmri_dataset` with input scans and mask, or use `nodata` for a dry run.
...
```{r afni_options_example, eval=FALSE}
...
# Example using AFNI's BLOCK basis (duration=2, penalty defaults to 1)
...
## Trialwise Modulation (`-stim_times_IM`)

For per-trial modulation (e.g., reaction time), use `afni_trialwise()`, which maps to `-stim_times_IM`.

```{r afni_trialwise_example, eval=FALSE}
# Add a continuous modulator (e.g., RT scaled 0.2–0.8 s)
set.seed(123)
simple_design$rt <- runif(nrow(simple_design), 0.2, 0.8)

emodel_tw <- event_model(
  onset ~ afni_trialwise(rt, basis = "gamma"),
  data = simple_design,
  block = ~ run,
  sampling_frame = sframe
)

fmodel_tw <- fmri_model(emodel_tw, bmodel)
alm_spec_tw <- afni_lm(fmodel_tw, dset)
print(alm_spec_tw)

# The command should include -stim_times_IM for the trialwise regressor.
```
...
```{r afni_censor_example, eval=FALSE}
...
## Dry Runs with `-nodata` and Design Inspection

You can validate command generation without data by providing `nodata = c(nTR, TR)`. Combine with `x1D_stop = TRUE` to write the design matrix (`.xmat.1D`) and stop before fitting.

```{r afni_nodata_example, eval=FALSE}
# Single run of 140 TRs with TR=2 sec
alm_spec_nodata <- afni_lm(
  fmodel,
  dataset = NULL,
  nodata = c(140, TR),
  x1D_stop = TRUE,
...
print(alm_spec_nodata)

# Then generate files/scripts without executing AFNI:
# run(alm_spec_nodata, outdir = "glm_afni_x1d", execute = FALSE)
```

Note on multi-run nodata: If your event model uses multiple runs (e.g., via `block = ~ run` in `event_model`) and you provide `nodata`, `afnireg` automatically adds a `-concat` string assuming equal run lengths. The run starts are computed from `nTR / n_runs` and added as `-concat '1D: 0 start2 start3 ...'`.

```{r afni_nodata_concat, eval=FALSE}
# Example: two runs implied by the event model, total of 280 TRs
alm_spec_nodata2 <- afni_lm(
  fmodel,
  dataset = NULL,
  nodata = c(280, TR)
)
cat(alm_spec_nodata2$cmd$cmd, "\n")
# Expect to see: -concat '1D: 0 140'
```

## Impulse Responses (`-iresp`)

Set `iresp = TRUE` in `afni_lm()` to have AFNI write impulse response estimates for `afni_hrf()` terms.

```{r afni_iresp_example, eval=FALSE}
alm_spec_iresp <- afni_lm(fmodel_afni, dset, iresp = TRUE)
print(alm_spec_iresp)
# Look for -iresp entries paired with -stim_times terms.
```
